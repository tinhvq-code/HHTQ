...
import { AuthInput, AuthTabs, SocialButton, authColors, authInputSx, getAllUsers, getStoredUsers, normalizeEmail } from './LoginScreen.js';
...
function RegisterScreen({ variant = 'empty' }) {
  ...
  const validate = () => {
    ...
    if (!nextErrors.email && getAllUsers().some((user) => normalizeEmail(user.email) === email)) {
      nextErrors.email = 'Email đã được đăng ký.';
    }
  ...
  const handleSubmit = (event) => {
    ...
    window.localStorage.setItem('registeredUsers', JSON.stringify([...getStoredUsers(), nextUser]));
    window.localStorage.setItem('currentUser', JSON.stringify({ id: nextUser.id, fullName: nextUser.fullName, email: nextUser.email }));
      ...
      >
        ...
        <Stack
          ...
          sx={{
            ...
            '& .MuiTypography-root': { fontSize: { xs: 11, md: 14 } },
            '& .MuiInputBase-root': { height: { xs: 29, md: 42 } },
            '& .MuiFormHelperText-root': { fontSize: 10, lineHeight: 1.15 }
          }}
        >
          <AuthInput
            label="Họ và tên"
          ...
          <Box>
            <Typography sx={{ color: '#8b8b8b', fontSize: 11, fontWeight: 700, mb: 0.55 }}>
              Giới tính
            </Typography>
            <TextField
              select
              ...
              FormHelperTextProps={{
                sx: { ml: 0, mt: 0.45, color: authColors.error, fontSize: 10, fontWeight: 700, lineHeight: 1.15 }
              }}
              InputProps={{
                startAdornment: (
        ...
        <Box sx={{ pt: 1.05 }}>
          {message && (
            <Typography sx={{ mb: 0.8, color: '#12b76a', fontSize: 11, fontWeight: 800 }}>
              {message}
            </Typography>
          )}
          ...
          <Button
            ...
            sx={{
              height: { xs: 34, md: 44 },
              bgcolor: hasValueOrError ? authColors.orange : authColors.mutedOrange,
              color: '#fff',
              borderRadius: 0.75,
