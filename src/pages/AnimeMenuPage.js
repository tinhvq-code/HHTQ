      ...
      <PhoneFrame>
        <Box sx={{ height: '100%', overflowY: 'auto', scrollbarWidth: 'none', backgroundColor: '#101010', color: '#fff', pb: 4 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', p: 2, pt: 3, position: 'sticky', top: 0, bgcolor: '#101010', zIndex: 10 }}>
            ...
            <Typography variant="h6" sx={{ ml: 1.5, fontWeight: 'bold' }}>Anime</Typography>
          </Box>
          ...
          <Typography sx={{ px: 2, mb: 2, fontWeight: 'bold', color: '#dcdcdc', fontSize: 16 }}>
            HÔM NAY XEM GÌ
          </Typography>

          <Box sx={{ px: 2, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1.5 }}>
            {animeList.map((item) => (
                ...
                <Box sx={{ position: 'relative', borderRadius: 1.5, overflow: 'hidden', aspectRatio: '2/3', mb: 1, '&:hover': { opacity: 0.8 } }}>
                  ...
                  {item.isNew && (
                    <Box sx={{ position: 'absolute', top: 6, left: 6, backgroundColor: '#e53935', px: 0.8, py: 0.3, borderRadius: '4px' }}>
                      <Typography sx={{ color: '#fff', fontSize: 9, fontWeight: 'bold' }}>Sắp chiếu</Typography>
                    </Box>
                  )}
                  
                  <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
                    <PlayCircleOutlinedIcon sx={{ fontSize: 32, color: 'rgba(255,255,255,0.9)' }} />
                  </Box>
                </Box>

                <Typography sx={{ fontWeight: 'bold', lineHeight: 1.3, fontSize: 11.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                  {item.title}
                </Typography>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', color: '#aaa', mt: 0.5 }}>
                  <Typography sx={{ fontSize: 10 }}>{item.eps}</Typography>
                  <Typography sx={{ fontSize: 10 }}>{item.views}</Typography>
                </Box>
              </Box>
            ))}
          </Box>
