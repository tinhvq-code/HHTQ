      ...
      <PhoneFrame>
        <Box sx={{ height: '100%', overflowY: 'auto', scrollbarWidth: 'none', backgroundColor: '#101010', color: '#fff', pb: 4 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', p: 2, pt: 3, position: 'sticky', top: 0, bgcolor: '#101010', zIndex: 10 }}>
            ...
            <Typography variant="h6" sx={{ ml: 1.5, fontWeight: 'bold', fontSize: 16 }}>
              Bảng Xếp Hạng
            </Typography>
          </Box>
          ...
          >
            <Tab label="BXH Phim Anime" sx={{ color: '#aaa', textTransform: 'none', fontWeight: 'bold', fontSize: 13 }} />
            <Tab label="BXH Truyện Tranh" sx={{ color: '#aaa', textTransform: 'none', fontWeight: 'bold', fontSize: 13 }} />
          </Tabs>
            ...
            {['Ngày', 'Tuần', 'Tháng', 'Năm'].map((filter) => (
              <Button
                ...
                sx={{
                  ...
                  borderRadius: 1, textTransform: 'none', minWidth: 'auto', px: 2, py: 0.5, fontSize: 12,
          ...
          <Box sx={{ px: 2 }}>
            {currentList.map((item) => (
              <Box key={item.rank} sx={{ mb: 3 }}>
                ...
                <Box sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden', mb: 1, aspectRatio: '16/9' }}>
                  <img src={item.img} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                  
                  {tabIndex === 0 && (
                    <Box sx={{
                      position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
                      backgroundColor: 'rgba(0,0,0,0.4)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 0.5
                    }}>
                      <PlayCircleOutlinedIcon sx={{ fontSize: 36, color: 'rgba(255,255,255,0.8)' }} />
                    </Box>
                  )}

                  {tabIndex === 1 && (
                    <Box sx={{
                      position: 'absolute', top: 8, left: 8, backgroundColor: 'rgba(0,0,0,0.6)', borderLeft: '2px solid #ff4444', borderRadius: '2px', px: 1, py: 0.2
                    }}>
                      <Typography variant="caption" sx={{ color: '#fff', fontWeight: 'bold', fontSize: 10 }}>
                        {item.chap}
                      </Typography>
                    </Box>
                  )}
                ...
                <Typography sx={{ fontWeight: 'bold', mb: 0.5, fontSize: 14 }}>
                  {item.title}
                </Typography>
                
                <Box sx={{ display: 'flex', justifyContent: 'space-between', color: '#aaa' }}>
                  {tabIndex === 0 && <Typography sx={{ fontSize: 12 }}>{item.eps}</Typography>}
                  <Typography sx={{ ml: tabIndex === 1 ? 0 : 'auto', fontSize: 12 }}>
                    {item.views}
                  </Typography>
                </Box>
              </Box>
